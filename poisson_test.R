...
# TODO Need to post-process with p.adjust
...
data.frame(strep_poisson[which(p_values < 0.05),], V4=p_values[which(p_values < 0.05)])
...
p_values <- p.adjust(p_values, n=2034)

data.frame(neisseria_poisson[which(p_values < 0.05),], V4=p_values[which(p_values < 0.05)])
